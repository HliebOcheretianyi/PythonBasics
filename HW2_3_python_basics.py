# -*- coding: utf-8 -*-
"""Копія записника "HW2_3_python_basics.ipynb"

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1bRl1gSl3x8Y-Mi1gOfbCIFnxiJJBxMB1
...
def digits_to_words(text):
    ...
    return ' '.join([digit_words[d] for d in digits]) if digits else ''
...
def get_work_time(df):
    submit_times = df['SubmitTime']
    accept_times = df['AcceptTime']

    df['WorkTime'] = submit_times - accept_times
...
def calculated_work_time(df):
    ...
    def calculate_span(group):
        earliest_accept = group['AcceptTime'].min()
        latest_submit = group['SubmitTime'].max()
        return latest_submit - earliest_accept

    return df_copy.groupby('WorkerId').apply(calculate_span)
...
def extract_revenue(df, column, name):
    def parse_money(money_str):
        try:
            ...
            money_str = money_str.split('[')[0].strip()
            amount = price_dec(money_str)
            return round(float(amount) / 1_000_000, 2) if amount is not None else 0
        except:
            return 0

    df_copy = df.copy()
    df_copy[name] = df_copy[column].apply(parse_money)
    return df_copy
